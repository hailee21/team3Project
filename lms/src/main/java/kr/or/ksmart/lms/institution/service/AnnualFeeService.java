...
import kr.or.ksmart.lms.institution.mapper.AnnualFeeMapper;
...
public class AnnualFeeService {
	@Autowired
	AnnualFeeMapper annualFeeMapper;
	...
	public Map<String, Object> getAnnualFee(String institutionCode) {
		...
		List<PaymentAnnualFee> paymentList = annualFeeMapper.selectPaymentAnnualFeeList(institutionCode);
		
		//교육원 사용권한 조회 select mapper 호출
		AvailableInstitution availableInstitution = annualFeeMapper.selectAvailableInstitution(institutionCode);
	...
	public Map<String, Object> getPaymentAnnualFee(String institutionCode) {
		...
		//컨트롤러로 리턴할 데이터 선언 및 설정
		Map<String, Object> returnMap = new HashMap<String, Object>();
		InfoAnnualFeeByPayment infoAnnualFee = annualFeeMapper.selectInfoAnnualFee();
	...
	public void addPaymentAnnualFee(PaymentAnnualFee paymentAnnualFee) {
		...
		String paymentAnnualFeePk = annualFeeMapper.selectPaymentAnnualFeePk(); //payment_annual_fee 테이블에서 마지막으로 입력된 PK 갑을 가져온다.
		...
		annualFeeMapper.insertPaymentAnnualFeey(paymentAnnualFee);
		...
		AvailableInstitution availableInstitution = annualFeeMapper.selectAvailableInstitution(paymentAnnualFee.getInstitutionCode());
		if(availableInstitution == null){
			...
			annualFeeMapper.insertAvailableInstitution(available);
		} else {
			...
			annualFeeMapper.updateAvailableInstitution(availableInstitution);
		}
	}
}
